""" Транспорт клиента. Обработчик сообщений поступающих с сервера. """
...
class ClientTransport(threading.Thread, QObject):
	...
	def process_server_ans(self, message):
		...
		# Если это сообщение от пользователя добавляем в базу, даём сигнал о новом сообщении
				...
				DESTINATION in message and MESSAGE_TEXT in message and \
	...
	def contacts_list_update(self):
		...
		CLIENT_LOGGER.debug(f'Запрос контакт листа для пользователся {self.name}, {self.client_name}')
		req = {
			ACTION: GET_CONTACTS,
			TIME: time.time(),
